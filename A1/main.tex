...
\begin{solution}
    \begin{enumerate}[(a)]
        ...
        \textit{(Aside)}Here is another construction which doesn't take into account the distribution of $r$:
        \begin{itemize}
            \item The adversary sends two messages $(m_{00}, m_{01})$ and $(m_1, m_1)$, none of $m_{00},m_{01},m_1$ are equal, and receives ciphertext pair $(ct_0, ct_1)$ from the challenger.
            \item The adversary decrypts the received ciphertexts with a random key. If he finds that $\dec(k,ct_0)=\dec(k,ct_1)=m_1$, then $\calA$ guesses $b' = 1$ else it sends $b' = 0$.
        \end{itemize}
        $$\Pr[b'=1|b=0]\approx0$$
        On the other hand, when b=1, 
        $$\Pr[b'=1|b=1]=2^{-l_2}$$
        Which gives a non-zero advantage.
        % The winning probability of $\calA$ in this scenario would be:
        %  $$\Pr[\text{Adversary wins}]=\frac{1}{2} + \frac{1}{2^{l_3 + 1}}$$. 
         
        %  NOTE: Two distinct messages can't have same ciphers with same random strings and two same messages with different random strings will not have the same cipher text. So,
        %   $$\mathsf{Adv}[\calA] =  \Big| \Pr[b'=1|b=0] - \Pr[b'=1|b=1] \Big| $$ 
        %   where 
        %   $$Pr[b'=1|b=0] = \frac{1}{2^{l_3}}$$ 
        %   and 
        %   $$Pr[b'=1|b=1] = 0$$
        ...
        % $$\mathsf{Adv}[\calA] =  \Big| Pr[b'=1|b=0] - Pr[b'=1|b=1] \Big| = \frac{1}{2} + \frac{1}{2^{l_3 + 1}}$$
