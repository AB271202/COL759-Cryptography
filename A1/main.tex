...
\begin{solution}
    \begin{enumerate}[(a)]
        ...
        But we can have randomized encryption shcemes with randomness drawn from $\bit^{l_3}$. Still there is a non-zero probability of $\frac{1}{2^{l_3}}$ of the two random strings selected to be same and thus the cipher texts will turn out to be same for two same messages. So the adversary performs the following : 
        \begin{itemize}
            \item The adversary sends two messages $(m_{00}, m_{01})$ and $(m_1, m_1)$ and receives cipher text pair $(ct_0, ct_1)$ along with the random strings.
            \item The adversary checks whether $ct_0 = ct_1$ under the constraint that the random strings are same. If that's the case, then $\calA$ guesses $b' = 1$ else it sends $b' = 0$.
        \end{itemize}
        The winning probability of $\calA$ in this scenario would be $\frac{1}{2} + \frac{1}{2^{l_3 + 1}}$. (NOTE: Two distinct messages can't have same ciphers with same random strings and two same messages with different random strings will not have the same cipher text. So Adv[$\calA$] = $ \Big| Pr[b'=1|b=0] - Pr[b'=1|b=1] \Big| $, where $Pr[b'=1|b=0] = \frac{1}{2^{l_3}}$ and $Pr[b'=1|b=1] = 0$)\\

        
        Even if the encryption scheme is such that it samples only distinct random strings for encryption, still there is non-zero probability of two different messages having the same cipher-texts. Note that two different messages can't have the same ciphers as this would lead to ambiguity in decryption. So in this case the adversary remains the same with only difference that it outputs 1 if $ct_0 = ct_1$ irrespective of random strings. In this case $Pr[b'=1|b=0] = 0$ (because two same messages with different random strings will not have the same cipher text.) and $Pr[b'=1|b=1] = \frac{1}{2^{l_3}}$. So Adv[$\calA$] = $ \Big| Pr[b'=1|b=0] - Pr[b'=1|b=1] \Big| = \frac{1}{2} + \frac{1}{2^{l_3 + 1}}$
