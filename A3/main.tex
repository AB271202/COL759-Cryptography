...
\begin{solution}
    ...
    %     \begin{itemize}
    %         \item The challenger $\calC$ samples a key $k$ and a random string $r$. It also maintains a table $\calT$ which is initially empty.
    %         \item The Adversary $\calA$ sends polynomially many queries to the challenger. For each query $(m_{i0}, m_{i1})$, the challenger responds by sending $(r_i, y_i \oplus m, F(k, m))$ to $\calA$, where $y_i \leftarrow \bit^n$ is sampled uniformly randomly on-the-fly by the challenger, and the tuple $(r_i, y_i)$ is stored in $\calT$. If $(r_i, .)$ is already present in $\calT$, then the challenger outputs the corresponding value of $r_i$ as $y_i$.
    %         \item Finally $\calA$ sends a bit $b'$ as its output to the challenger.
    %     \end{itemize}
    \textbf{Hybrid-World-b:} 
        \begin{itemize}
...
\begin{solution}
    \begin{enumerate}[(a)]
        ...
        So overall there are $2^n$ expected number of pairs of $k1$ \text{and} $k2$ satisfying our equation. \\ \\ 
